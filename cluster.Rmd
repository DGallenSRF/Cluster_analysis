...
First we filter out the weekends. We already determined that they will have their own cluster. 
...
```{r }
#run kmeans
speed_cluster <-  kmeans(cluster_dat_midweek%>%select(Speed,YearDay,weather)%>%scale(),centers = 4,nstart = 10)
...
Fist a simple 2d plot

```{r 2d plots}
plotly::plot_ly(cluster_dat_midweek,x=~YearDay,y=~Speed,color=~cluster,type='scatter',mode='markers',text=~paste('Weekday:',Weekday))
```

The 2d plot does not do a good job of showing where the clusters split.

we will try a 3d plot.
...
We now create a plot that breaks the clusters down by weekday (or whatever we want).

```{r plot split up by weekday}
