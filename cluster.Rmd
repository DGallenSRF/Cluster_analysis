...
#run kmeans
speed_cluster <-  kmeans(cluster_dat_midweek%>%select(Speed,YearDay,weather)%>%scale(),centers = 5,nstart = 20)

##add cluster to fitlered dataframe
cluster_dat_midweek$cluster <- as.factor(speed_cluster$cluster)

plotly::plot_ly(cluster_dat_midweek,x=~Time_Bin,y=~Speed,color=~cluster,type='scatter',mode='markers',
