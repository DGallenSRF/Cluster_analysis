...
##add cluster to fitlered dataframe
#cluster_dat_midweek$cluster <- as.factor(speed_cluster$cluster)
dat_scale <- cluster_dat_midweek%>%select(Time_Bin,Speed,YearDay,weather)%>%
  mutate(Speed = scale(Speed),
         YearDay = scale(YearDay),
         weather = scale(weather))
dat_scale$cluster <- as.factor(speed_cluster$cluster) 
dat_scale <-  melt(dat_scale,id = c('Time_Bin','cluster'))

centers <- as.data.frame(speed_cluster$centers)
centers$cluster <- seq(1,nrow(centers),1)
centers <- melt(centers,id='cluster')

j <- merge(centers,dat_scale,by=c('cluster','variable'))%>%mutate((value.x-value.y)^2)
ja <- j %>% group_by(cluster,variable)%>%slice(which.min(value.x-value.y))
